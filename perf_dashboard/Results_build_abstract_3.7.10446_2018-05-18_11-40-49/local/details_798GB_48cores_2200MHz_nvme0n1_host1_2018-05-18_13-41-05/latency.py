...
# nvmef_perf.py 1.0
# Suman Debnath
# suman.debnath@toshiba-tsip.com
#
#
# ------------------------------------------------------------------------
# This tool is based on ezfio which is free software, and we added/removed
# tests/features based on our need to run it on our targets
# ------------------------------------------------------------------------
#
...
def CollectDriveInfo():
    ...
    global physDriveGiB, physDriveGB, physDriveBase, testcapacity, each_drive_size
    global model, serial, physDrive, no_drives, each_drive_size
    # We absolutely need this information
    try:
        if ":" in physDrive:
            no_drives = len(physDrive.split(":"))
            all_dev = physDrive.split(":")[0]
        else:
            no_drives = 1
            all_dev = physDrive

        physDriveBase = os.path.basename(all_dev)
        code, physDriveBytes, err=Run(['blockdev', '--getsize64', all_dev])
        if code != 0:
            raise Exception("Can't get drive size for " + all_dev)
        physDriveGB = ((long(physDriveBytes))/(1000 * 1000 * 1000)) * no_drives
        physDriveGiB = ((long(physDriveBytes))/(1024 * 1024 * 1024)) * no_drives
        testcapacity = (physDriveGiB * utilization) / 100
        each_drive_size = physDriveGiB / no_drives
    except:
...
def SetupFiles():
    ...
    def CSVInfoHeader(f):
        ...
        AppendFile("Targets," +str(no_drives), f)
        AppendFile("EachTargetSize," + str(each_drive_size) + ",GiB", f)
    ...
    if os.path.exists(testcsv):
        os.unlink(testcsv)
    # Disabling the CSV Header
    # CSVInfoHeader(testcsv)
    AppendFile("Type,Write %,Block Size,Threads,Queue Depth/Thread,IOPS," +
               "Bandwidth (MB/s),Read Latency (us),Write Latency (us),Timestamp,Timestamp_ms, Time_date, Test_Name",
               testcsv)
...
def RunTest(iops_log, seqrand, wmix, bs, threads, iodepth, runtime, tname):
    ...
    if skiptest:
        ...
        time_stamp = dd.fromtimestamp(time_stamp).strftime('%Y-%m-%d_%H-%M-%S')
        time_stamp_ms = j["timestamp_ms"]
        time_date = j["time"]
    ...
    AppendFile( ",".join((str(seqrand), str(wmix), str(bs), str(threads),
                          ...
                          str(wlat), str(time_stamp), str(time_stamp_ms), str(time_date), str(tname))), testcsv)
...
#                lambda o: { AppendFile(o['name'], testcsv ) } )
    ...
    testname = "Latency Test Sustained 4KB Random Read Tests by Number of Threads"
    seqrand = "Rand"
    wmix=0
    bs=4096
    runtime=shorttime
    ...
    testname = "Latency Test Sustained 4KB Random Write Tests by Number of Threads"
    seqrand = "Rand"
    wmix=100
    bs=4096
    runtime=shorttime
...
def RunAllTests():
    ...
    global ret_iops, ret_mbps, ret_lat, fioVerString, each_drive_size
    ...
    print "Welcome to KumoScale Appliance Performance QA Regression Suite\n"
    print "*" * len(fmtstr.format("", "", "", ""))

    fmtinfo="{0: >20}: {1}"
    print fmtinfo.format("Drives", str(physDrive))
    print fmtinfo.format("Model", str(model))
    print fmtinfo.format("Target Type", str(serial))
    print fmtinfo.format("AvailCapacity", str(physDriveGiB) + " GiB")
    print fmtinfo.format("TestedCapacity", str(testcapacity) + " GiB")
    print fmtinfo.format("Targets", str(no_drives))
    print fmtinfo.format("EachTargetSize", str(each_drive_size) + " GiB")
...
"""
    ...
    def CombineExceedanceCSV():
        ...
        files = []
        for qd in [ 1, 4, 16, 32 ]:
            r = open( TestName("Rand", 50, 4096, qd, 1) + ".exc.read.csv" )
            w = open( TestName("Rand", 50, 4096, qd, 1) + ".exc.write.csv" )
            files.append( [ r, w ] )
        while True:
            all_empty = True
            l = ""
...
each_drive_size = "" # Each Drive/Target size
no_drives = "" # No. of Targets
